...
from pyomo.environ import ConcreteModel, Set, RangeSet, Param, Reals, Binary, Integers, Objective, maximize, Any
...
class FantasyOptimizer():
    ...
    def build_best_formation_model(self, players, budget=100.0, bench_boost=False):
        ...
        def init_formations(m, j):
            ...
            if j == 7:
                return (5, 2, 3)
        model.Formations = Param(
            model.J, initialize=init_formations, within=Any)
        ...
        # Players on the pitch in formation j
        model.on_pitch = Var(model.I, model.J, within=Binary)
        model.cap = Var(model.I, within=Binary)  # Captain
        model.form_points = Var(model.J, within=Reals)  # Points in formation j
        # The best formation to use (max points)
        model.best_form = Var(model.J, within=Binary)
        # The number of players kept from the existing team (if given)
        model.n_keep = Var(within=Integers, bounds=(0, 15))
        # The number of changes from the existing team (if given)
        model.n_changes = Var(within=Integers, bounds=(0, 15))
    ...
    def required_players(self, required_players):
        ...
        index = [i for i, p in enumerate(
            self.players) if p in required_players]
    ...
    def solve_model(self):
        ...
        for j in model.J:
            if model.best_form[j].value >= 0.999:
                ...
                if self.opt_form is not None:
                    print("Ooops, multiple formations chosen!",
                          j, model.best_form[j].value)
        ...
        if N_cap != 1 or N_chosen != 15:
            print(
                f"OOPS, {N_chosen} players chosen with {N_cap} captains selected!")

    def display_results(self):

        if len(self.chosen_players) != 15 or self.captain is None:
            print(
                f"Full team not selected, number of players: {len(self.chosen_players)}, cptain: {self.captain}")
            return
        ...
        print("Captain:")
        print(self.captain.name, self.captain.team,
              self.captain.cost, self.captain.score)
        print("")
...
class MultiWeekFantasyOptimizer():
    ...
    def __init__(self, cplex_path: str) -> None:

        self.chosen_players: List['Player'] = []
        self.solver: SolverFactory = self.init_solver(cplex_path)
        self.model: Optional[ConcreteModel] = None

    def init_solver(self, cplex_path: str) -> SolverFactory:
    ...
    def build_best_formation_model(self, players: List['Player'], n_weeks: int, budget: float, existing_team: List['Player'], n_free_transf: int, n_max_transf: int = 15) -> None:
        ...
        # Players in squad (15)
        model.in_squad = Var(model.I, model.W, within=Binary)
        # Players on the pitch in formation j
        model.on_pitch = Var(model.I, model.J, model.W, within=Binary)
        model.cap = Var(model.I, model.W, within=Binary)  # Captain
        # Points in formation j
        model.form_points = Var(model.J, model.W, within=Reals)
        # The best formation to use (max points)
        model.best_form = Var(model.J, model.W, within=Binary)
        # The number of changes from the previous team
        model.n_changes = Var(model.W, bounds=(0, n_max_transf))
        # The number of free changes this round
        model.n_free = Var(model.W, bounds=(0, 15))
        # The number of costly changes this round
        model.n_costly = Var(model.W, bounds=(0, 15))
        # 1 if more than 1 change is made this round, 0 otherwise
        model.u = Var(model.W, within=Binary)
        # Player subbed into team
        model.subbed_in = Var(model.I, model.W, bounds=(0, 1))
        # Player subbed out of team
        model.subbed_out = Var(model.I, model.W, bounds=(0, 1))
        ...
        model.MaxPerTeam = Constraint(
            model.T, model.W, rule=max_per_team_constraint)
        ...
        model.OnPitchBounds = Constraint(
            model.I, model.J, model.W, rule=on_pitch_bounds)
        ...
        model.CapBounds = Constraint(
            model.I, model.J, model.W, rule=cap_bounds)
        ...
        def objective_rule(m):
            ...
            form_score = sum(m.Weight[w]*m.form_points[j, w]
    ...
    def add_min_team_players(self, team: str, n: int) -> None:
